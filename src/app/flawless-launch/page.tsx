...
export default function FlawlessLaunchPage() {
  ...
  const handleSelectProject = (projectId: string) => {
    ...
    router.replace(`/flawless-launch?project=${encodeURIComponent(projectId)}`);
  };

  const handleBackToOverview = () => {
    setSelectedProjectId(null);
    router.replace('/flawless-launch');
  };
  ...
  return (
    <div className="space-y-6 font-sans text-slate-800">
      ...
      <div className="p-4 rounded-2xl bg-white border border-slate-200 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/flawless-launch/dashboard"
            className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 hover:bg-emerald-100 text-xs font-bold transition shadow-xs"
          >
            <LayoutDashboard className="h-3.5 w-3.5" />
            <span>FLM Dashboard</span>
          </Link>
          <div className="hidden sm:flex items-center gap-2 text-xs text-slate-500">
            <Rocket className="h-4 w-4 text-emerald-600" />
            <span>5 Controlled FLM Indicators</span>
          </div>
        </div>
      </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Select an active program to evaluate its gate-by-gate performance across On-Time PPAP, Rebill, OEE, OTD, and OI%.
              </p>
            </div>
            ...
            {filteredProjects.map((p: Project) => {
              ...
              return (
                <div
                  ...
                  className="group bg-white p-5 rounded-2xl border border-slate-200/80 hover:border-emerald-400 hover:shadow-md transition-all cursor-pointer flex flex-col justify-between space-y-4"
                >
                  ...
                  <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
                    ...
                    <span className="text-emerald-600 font-bold flex items-center gap-1 group-hover:translate-x-0.5 transition-transform text-[11px]">
                      View FLM
