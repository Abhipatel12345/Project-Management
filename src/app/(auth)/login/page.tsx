...
function LoginForm() {
  ...
  return (
    <div className="min-h-screen bg-[#090E1A] flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-md bg-[#0F172A]/90 border border-slate-800 rounded-3xl p-8 shadow-2xl backdrop-blur-xl relative z-10 space-y-6">
        ...
        <div className="text-center space-y-2">
          <div className="inline-flex h-12 w-12 rounded-2xl bg-sky-600 items-center justify-center shadow-lg shadow-sky-600/30 mb-2">
            <Car className="h-6 w-6 text-white" />
          </div>
          <div className="flex items-center justify-center gap-1.5 text-xs font-bold text-sky-400 uppercase tracking-widest">
            <span>NETLINK AUTOMOTIVE PDM</span>
          </div>
          <h1 className="text-2xl font-black tracking-tight text-white">Enterprise Gateway</h1>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className={cn(
                  'w-full pl-10 pr-4 py-2.5 text-xs font-medium bg-[#070B15] border rounded-xl text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-1 transition',
                  errors.usr
            ...
            {errors.usr && (
              <p className="text-[11px] text-rose-400 font-bold">{errors.usr.message}</p>
            )}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="••••••••••••"
                className={cn(
                  'w-full pl-10 pr-10 py-2.5 text-xs font-medium bg-[#070B15] border rounded-xl text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-1 transition',
                  errors.pwd
                    ? 'border-rose-500/60 focus:ring-rose-500'
                    : 'border-slate-800 focus:border-sky-500 focus:ring-sky-500/30'
                )}
            ...
            {errors.pwd && (
              <p className="text-[11px] text-rose-400 font-bold">{errors.pwd.message}</p>
            )}
          </div>

          {/* Remember Me */}
          <div className="flex items-center justify-between text-xs pt-1">
            <label className="flex items-center gap-2 cursor-pointer text-slate-400 hover:text-slate-200 font-medium">
              <input
                {...register('rememberMe')}
                type="checkbox"
                className="rounded border-slate-800 bg-[#070B15] text-sky-500 focus:ring-sky-500/20"
              />
              <span>Remember session</span>
            </label>
            <a href="#forgot" className="text-sky-400 hover:underline font-bold">
              ERPNext Help
            </a>
          </div>
          ...
          <button
            ...
            className="w-full py-3 px-4 text-xs font-bold rounded-xl bg-sky-600 hover:bg-sky-500 text-white transition shadow-lg shadow-sky-600/30 flex items-center justify-center gap-2 disabled:opacity-50 mt-2 cursor-pointer"
          >
            {isSubmitting ? (
              <>
